...
<header>
  <h1>__TITLE__</h1>
  <div class="hint">Use ← / → / R to step • Generated from __SOURCE__</div>
</header>
...
QUOTED = r'"(?:\\.|[^"])*"'
PAIR_RE = re.compile(r'(\w+)\s*=\s*(' + QUOTED + r'|\S+)')
DIR_RE  = re.compile(r'//\s*@(\w+)(.*)$')

def parse_directives(line: str):
    ...
    if kind in ('perf','anno'):
        q = re.search(QUOTED, rest)
        val = json.loads(q.group(0)) if q else rest
        return {'kind': kind, 'args': {'_value': val}}
...
def directive_to_action(d):
    k, a = d['kind'], d['args']
    getv = lambda A: A.get('val') if A.get('val') is not None else A.get('value')

    if k == 'label':
        return {'t':'label','name':a.get('name')}

    if k == 'call':
        return {'t':'call','target': a.get('target') or a.get('func'), 'arg': a.get('arg')}

    if k == 'ret':
        return {'t':'ret','func': a.get('func')}

    if k == 'tailcall':
        return {'t':'tailcall','target': a.get('target') or a.get('func'), 'arg': a.get('arg')}

    if k == 'ret_auto':
        return {'t': 'ret_auto', 'func': a.get('func')}

    if k == 'unwind':
        # either count=N or func=NAME
        return {'t':'unwind', 'count': a.get('count'), 'func': a.get('func')}

    if k == 'unwind_all':
        return {'t':'unwind_all', 'func': a.get('func')}

    if k == 'clear_stack':
        return {'t': 'clear_stack'}

    if k in ('push','push_stack'):
        return {'t':'push_stack','name':a.get('name'),'val': getv(a), 'info': a.get('info')}

    if k == 'pop_stack':
        return { 't': 'pop_stack', 'name': a.get('name'),
                 'id': a.get('id'), 'count': a.get('count') }

    if k in ('update','update_stack'):
        return {'t':'update_stack','name':a.get('name'),'val': getv(a)}

    if k in ('alloc','alloc_heap'):
        return {'t':'alloc_heap','id':a.get('id'),'label':a.get('label',''),
                'size':a.get('size'),'meta':a.get('meta')}

    if k in ('free','free_heap'):
        return {'t':'free_heap','id':a.get('id')}

    if k == 'leak':
        return {'t':'leak','id':a.get('id')}

    if k == 'sp_alloc':
        return {'t':'sp_alloc','id':a.get('id'),'label':a.get('label',''),
                'uc':a.get('uc',1),'size':a.get('size')}

    if k == 'sp_inc':       return {'t':'sp_inc','id':a.get('id')}
    if k == 'sp_dec':       return {'t':'sp_dec','id':a.get('id')}
    if k == 'sp_free':      return {'t':'sp_free','id':a.get('id')}
    if k == 'sp_cycle_leak':return {'t':'sp_cycle_leak','ids': a.get('ids',[])}
    if k == 'bind_ptr':     return {'t':'bind_ptr','name': a.get('name'), 'id': a.get('id')}
    if k == 'unbind_ptr':   return {'t':'unbind_ptr','name': a.get('name')}
    return None

def infer_perf(src: str):
    s = src.strip()
    if 'std::make_unique' in s or re.search(r'\\bnew\\b', s):
        return 'heap: allocator work'
    if 'std::make_shared' in s:
        return 'heap: allocate control block + object'
    if 'delete' in s:
        return 'heap: free'
    if re.match(r'(int|float|double|char)\\s+[\\w\\*\\[\\]]+', s):
        return 'stack: pointer bump + store'
...
def build_program(lines):
    ...
    for raw in lines:
        ...
        if d:
            if d['kind'] in ('perf','anno'):
                entry[d['kind']] = d['args'].get('_value','')
            else:
                entry['a'] = directive_to_action(d)
        if 'perf' not in entry:
            entry['perf'] = infer_perf(src)
        program.append(entry)
    return program

def main():
    ap = argparse.ArgumentParser(description='Generate interactive Memory Slide HTML from C++ snippet with directives')
    ap.add_argument('cpp', help='Input C++ file')
    args = ap.parse_args()

    lines = sys.stdin.readlines()
    program = build_program(lines)
    source_name = pathlib.Path(args.cpp).name
    title = "Demo Slide"
    ...
    html = (TEMPLATE
            ...
            .replace('__PROGRAM__', json.dumps(program, ensure_ascii=False)))
